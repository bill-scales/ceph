...
protected:
  ...
  void verify_logs(bool identical = false, bool tails_different = false)
  {
	...
	EXPECT_EQ(log.get_tail(), logp.get_tail());
      ...
      dout(0) << "BILL logp head " << logp.get_head() << " tail " << logp.get_tail() << dendl;
      for (auto entry : logp.get_log().log) {
	dout(0) << "BILL logp entry " << entry.version << dendl;
      }
      dout(0) << "BILL log head " << log.get_head() << " tail " << log.get_tail() << dendl;
      for (auto entry : log.get_log().log) {
	dout(0) << "BILL log entry " << entry.version << dendl;
      }
      while (pi != pe && i != e) {
